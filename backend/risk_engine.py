...
def build_action_checklist(level: str, hazard_class: str, hot_work_active: bool, confined_space_active: bool) -> List[str]:
    """
    Hazard-aware action checklist — a judge-readable expansion of
    RECOMMENDED_ACTION. Deterministic (rule-based on level + hazard_class +
    active permit types), not a generative model — every item traces directly
    to a condition below, so it stays reproducible and explainable.
    """
    if level == "LOW":
        return ["Routine monitoring — no action required."]
    if level == "MODERATE":
        items = ["Increase monitoring frequency", "Notify shift supervisor"]
        if hot_work_active:
            items.append("Review hot-work permit conditions")
        return items

    items: List[str] = []
    items.append("Evacuate zone" if level == "CRITICAL" else "Restrict non-essential entry to zone")
    if hot_work_active:
        items.append("Stop hot work")
    if "fire" in hazard_class:
        items.append("Shut down ignition sources")
        if level == "CRITICAL":
            items.append("Dispatch fire response team")
    if "toxic_gas" in hazard_class or "confined_space" in hazard_class or confined_space_active:
        items.append("Start forced ventilation")
    items.append("Notify safety officer immediately" if level == "CRITICAL" else "Notify safety officer")
    if level == "CRITICAL":
        items.append("Suspend all active permits in zone")
    return items
...
def assess_zone(
    ...
    return ZoneAssessment(
        ...
        action_checklist=build_action_checklist(
            level, hazard_class, permit_state["hot_work_active"], permit_state["confined_space_active"]
        ),
    )
    ...
    for zone_id in simulator.zones:
        ...
        results.append(
            assess_zone(
                ...
                hazard_class=simulator.zones[zone_id].hazard_class,
            )
        )
